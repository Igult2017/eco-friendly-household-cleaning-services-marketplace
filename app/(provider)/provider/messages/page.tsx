...
export default async function ProviderMessagesPage() {
  ...
  // Recent messages on this cleaner's bookings, joined to the client. Grouped per booking in JS:
  // first row per booking (desc order) is the latest message; unread = client messages not yet read.
  const rows = await db
    .select({
