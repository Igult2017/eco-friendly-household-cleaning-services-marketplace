...
export default async function OnboardingLayout({ children }: { children: React.ReactNode }) {
  ...
  // Provide the client with the per-locale messages so client `useTranslations`
  // calls in the onboarding subtree render properly. `getMessages()` reads the
  // request's locale (already set by the parent locale middleware/layout) and
  // returns the full messages object for the current locale.
