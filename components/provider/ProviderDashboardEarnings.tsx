...
export function ProviderDashboardEarnings({
  totalEarnings,
  pendingPayout,
  recentPayouts,
}: {
    ...
    <div className="bg-white rounded-2xl border border-[#E5EBF0] overflow-hidden">
      <div className="px-5 py-4 border-b border-[#F0F4F8] flex items-center justify-between">
        <h2 className="font-semibold text-[#2B3441] flex items-center gap-2">
          <TrendingUp size={16} className="text-[#2D7A5F]" /> Earnings
        </h2>
        <Link href="/provider/earnings" className="text-xs text-[#2D7A5F] hover:underline">Full report →</Link>
      </div>
      ...
      <div className="grid grid-cols-2 divide-x divide-[#F0F4F8] border-b border-[#F0F4F8]">
        ...
        <div className="px-5 py-4">
          <p className="text-xs text-[#6B7280] mb-1">Awaiting payout</p>
          <p className={cn("text-xl font-bold", pendingPayout > 0 ? "text-[#2D7A5F]" : "text-[#9CA3AF]")}>
            {formatCurrency(pendingPayout)}
          </p>
          {pendingPayout > 0 && (
            <p className="text-xs text-[#9CA3AF] mt-0.5">Paid weekly on Monday</p>
          )}
        </div>
      </div>
      ...
      {recentPayouts.length > 0 ? (
        ...
        <p className="px-5 py-4 text-sm text-[#9CA3AF]">No payouts yet — first payout runs after your first completed job.</p>
      )}
    </div>
  )
}
