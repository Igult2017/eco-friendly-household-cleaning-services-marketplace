    ...
    <div className="bg-white rounded-2xl border border-[#E5EBF0] overflow-hidden">
      ...
      <div className="grid grid-cols-2 divide-x divide-[#F0F4F8] border-b border-[#F0F4F8]">
        <div className="px-5 py-4">
          ...
          <p className="text-xl font-bold text-[#2B3441]">{formatCurrencyForCountry(totalEarnings, country)}</p>
        </div>
        <div className="px-5 py-4">
          ...
          <p className={cn("text-xl font-bold", pendingPayout > 0 ? "text-[#2D7A5F]" : "text-[#9CA3AF]")}>
            {formatCurrencyForCountry(pendingPayout, country)}
          </p>
          {pendingPayout > 0 && (
          ...
          {recentPayouts.map((p) => {
            ...
            return (
              <div key={p.id} className="px-5 py-3 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-[#2B3441]">{formatCurrencyForCountry(p.amount, country)}</p>
