...
const STATUS_CFG: Record<string, { labelKey: string; cls: string }> = {
  open:               { labelKey: "statusOpen",             cls: "bg-red-100 text-red-700" },
  under_review:       { labelKey: "statusUnderReview",      cls: "bg-amber-100 text-amber-700" },
  escalated:          { labelKey: "statusEscalated",        cls: "bg-red-100 text-red-700" },
  resolved_customer:  { labelKey: "statusResolvedCustomer", cls: "bg-orange-100 text-orange-700" },
  resolved_provider:  { labelKey: "statusResolvedProvider", cls: "bg-green-100 text-green-700" },
  closed:             { labelKey: "statusClosed",           cls: "bg-gray-100 text-gray-500" },
}
...
export async function ProviderDashboardDisputes({ disputes }: { disputes: DisputeRow[] }) {
  const t = await getTranslations("compProviderProviderDashboardDisputes")
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E5EBF0] overflow-hidden">
      <div className="px-5 py-4 border-b border-[#F0F4F8] flex items-center justify-between">
        ...
        <Link href="/provider/bookings" className="text-xs text-[#2D7A5F] hover:underline">{t("bookingsLink")}</Link>
      </div>
        ...
        {disputes.map((d) => {
          ...
          return (
            <div key={d.id} className={cn("px-5 py-4", isActive && "bg-red-50/40")}>
              ...
              {d.resolution && (
                <p className="text-xs text-[#2D7A5F] font-medium">{t("resolutionLabel", { resolution: d.resolution })}</p>
              )}
      ...
      {activeCount === 0 && disputes.length > 0 && (
        <div className="px-5 py-3 bg-[#F4FAF6] text-xs text-[#2D7A5F] font-medium flex items-center gap-1.5">
          <ShieldCheck size={12} /> {t("allResolved")}
        </div>
      )}
    </div>
  )
