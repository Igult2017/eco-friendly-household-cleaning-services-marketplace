    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <h2 className="font-serif text-xl font-bold text-[#2B3441]">Reschedule Booking</h2>
        </div>

        <p className="text-sm text-[#6B7280] mb-5">
          Current schedule: <span className="font-medium text-[#2B3441]">{new Date(currentScheduledAt).toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
        </p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#2B3441] mb-1">New Date</label>
            <input
              type="date"
              value={newDate}
              min={minDateStr}
          ...
          <div>
            <label className="block text-sm font-medium text-[#2B3441] mb-1">New Time</label>
            <input
              type="time"
              value={newTime}
          ...
          <div>
            <label className="block text-sm font-medium text-[#2B3441] mb-1">Duration</label>
            <select
              value={durationHours}
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {submitting ? <><Loader2 size={15} className="animate-spin" />Rescheduling…</> : "Reschedule"}
            </button>
          </div>
        </form>
      </div>
